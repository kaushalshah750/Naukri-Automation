...
const keywords = ['Angular', 'HTML', 'CSS', '.Net Core', 'UI/UX', 'JavaScript', 'Frontend', 'TypeScript', 'Node.js', 'Software Developer'];
const excludeKeywords = ["Java", "PHP", "AEM"];
let jobData = [];
...
async function takeScreenshot(driver, jobTitle, step) {
    ...
    const folderPath = path.join(__dirname, "Screenshot", "Search Job", safeJobTitle);
...
function saveToExcel() {
    // Define the base screenshot folder
    const baseDir = path.join(__dirname, 'Excel Data', "Search Job");
...
(async function naukriRecommendedJobs() {
    try {
        ...
        // await driver.get('https://www.naukri.com/angular-developer-html-and-css-developer-frontend-development-ui-development-front-end-jobs-in-pune?jobAge=1');
        // await driver.get('https://www.naukri.com/angular-developer-html-and-css-developer-frontend-development-ui-development-front-end-jobs-in-pune');
        await driver.get('https://www.naukri.com/angular-developer-html-and-css-developer-frontend-development-ui-development-front-end-jobs-in-pune?k=angular%20developer%2C%20html%20and%20css%20developer%2C%20frontend%20development%2C%20ui%20development%2C%20front%20end&l=pune&nignbevent_src=jobsearchDeskGNB&experience=3&jobAge=1');
        ...
        // Get job elements
        let page = await driver.findElement(By.xpath('//*[@id="jobs-list-header"]/div[1]/span')).getText();

        let totalPage = Math.ceil((page.split("of ")[1]) / 20);
        ...
        for (let i = 1; i < totalPage; i++) {
            ...
            await driver.wait(until.elementLocated(By.className('srp-jobtuple-wrapper')), 10000);
            let jobElements = await driver.findElements(By.className('srp-jobtuple-wrapper'));

            for (let job of jobElements) {
                let title = await job.findElement(By.className('title')).getText();
                let company = await job.findElement(By.className('comp-name')).getText();
                let location = await job.findElement(By.className('loc')).getText();
                let experience = await job.findElement(By.className('exp')).getText();
                let description = await job.findElement(By.className('job-desc')).getText();
                let jobLink = "";
                let summary = "";
                ...
                if (jobMatches && !containsExcludedKeyword) {
                    ...
                    if (companySiteApplyButton.length > 0) {
                        ...
                        jobData.push({ title, company, location, experience, description, jobLink, summary });
            ...
            await driver.get(`https://www.naukri.com/angular-developer-html-and-css-developer-frontend-development-ui-development-front-end-jobs-in-pune-${i+1}?jobAge=1`);
